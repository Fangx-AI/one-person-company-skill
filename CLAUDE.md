...
scripts/
  ...
  backup-db.js           # 热备份 + gzip + 轮转（cron 每 4h 跑）
  deploy.sh              # 一键部署（git pull / npm ci / migrate / pm2 reload / smoke）
  smoke-*.js             # auth / persistence / northstar 三个流程冒烟
  e2e-full-flow.js       # 30 个断言的端到端覆盖
...
### 4.4 claimAnonSessions：登录时自动归户

用户匿名期间（只有 anon cookie）的所有 chat 落在 `chat_sessions.anon_id` 上。
登录成功时 `routes/auth.js` 调 `users.js::claimAnonSessions(userId, anonId)`，
把那些 session 转挂到 `user_id`。

**这是产品级的关键体验**——别动这个 join 逻辑，不然用户会发现登录前的对话神秘消失。

但跨设备/跨 cookie 的 localStorage 对话**不会**自动同步——那个 `import-local-session`
接口已经被撤回（用户原话："其他大模型登录后数据就是会丢失的，没人做这个同步"）。
未来想加回来请先和用户确认。
...
跑 `test:e2e` 需要先开一个隔离 dev server，参数见 `scripts/e2e-full-flow.js` 顶部
...
`scripts/deploy.sh` 自带 10 步：预检 → 备份 DB → git pull → npm ci → 迁移
...
`PRAGMA` 不能在事务里执行。`scripts/migrate.js` 已经做了过滤，但如果你手写 SQL
...
5. ⏳ Aliyun key 永久删除（运维事项）

详见 git log：`git log --grep="P0"`。
...
3. `scripts/e2e-full-flow.js`（30 个断言告诉你产品契约是什么）
